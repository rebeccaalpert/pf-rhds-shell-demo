...
const DualListSelectorListWrapperBase = (_a) => {
    var { className, children, 'aria-labelledby': ariaLabelledBy, innerRef, options = [], selectedOptions = [], displayOption, id: idProp, isDisabled = false } = _a, props = tslib_1.__rest(_a, ["className", "children", 'aria-labelledby', "innerRef", "options", "selectedOptions", "displayOption", "id", "isDisabled"]);
    const generatedId = (0, helpers_1.useSSRSafeId)('dual-list-selector-list');
    ...
    const handleKeys = (event) => {
        ...
        const validOptions = isTree
            ? Array.from(menuRef.current.querySelectorAll(`.${dual_list_selector_1.default.dualListSelectorItemToggle}, .${dual_list_selector_1.default.dualListSelectorItemCheck} > input`)).filter((item) => !item.closest(`.${dual_list_selector_1.default.dualListSelectorList}[inert]`))
